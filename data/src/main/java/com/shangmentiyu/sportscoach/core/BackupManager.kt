...
object BackupManager {
    ...
    fun restoreDetailed(
        ...
        AppDatabase.closeAndResetInstance(context)
        // === v46 修复：强制短暂休眠，确保 SQLite 文件锁/句柄彻底释放 ===
        // Android 10+ 上 WAL 模式关闭后文件句柄释放可能异步，
        // 立即删除/覆盖 db 文件会报 "Permission denied / 文件被占用"，导致恢复失败。
        // 在解压覆盖前让出调度，等待 OS 完成句柄回收。
        try {
            Thread.sleep(100)
                    ...
                    when {
                        ...
                        entry.name.startsWith(ZIP_ENTRY_PHOTOS_DIR) -> {
                            ...
                            if (photoName.isNotBlank()) {
                                // === v46 安全修复：ZIP Slip 路径穿越防护 ===
                                // 恶意备份包可携带 "SignPhotos/../../databases/xxx" 等条目，
                                // 未过滤时会把文件写到 filesDir 之外（覆盖 app 沙箱任意文件）。
                                // 三重防护：拒绝 ".." / 绝对路径 / 反斜杠，并做 canonicalPath 前缀校验。
