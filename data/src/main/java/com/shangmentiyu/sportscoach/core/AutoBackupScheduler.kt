...
object AutoBackupScheduler {
    ...
    fun notifyDataChange() {
        if (!initialized) {
            // === v46 修复：未初始化时留痕，便于定位"自动备份失效" ===
            // 若 init 未执行（Application 初始化链路异常/时序问题），所有变更通知都会
            // 被静默吞掉 → 自动备份看似"失效"。此处输出高频可控的 W 日志辅助诊断。
            Log.w(TAG, "自动备份调度器尚未初始化，忽略本次数据变更通知")
            return
        }
        backupScope.launch {
    ...
    private suspend fun performAutoBackupIfNeeded() {
        // === v46 日志追踪：确认自动备份是否触发 ===
        // 在 Logcat 过滤 "AutoBackup" 即可看到每次防抖触发的时间戳
        Log.d("AutoBackup", "自动备份定时任务已被触发，当前时间戳: ${System.currentTimeMillis()}")
