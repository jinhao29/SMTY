...
class StudentRepository(
     ...
     *
     * 业务子表（Lesson / Schedule / LessonPackage / TrainingCycle）在事务内
     * 级联物理清理（双通道：studentId 优先，旧数据 NULL 回退姓名），避免 UI
     * 角标与统计残留"幽灵数据"；student_plan_images 无 studentId 字段，
     * 仅按姓名清理，已包含在 [deleteStudent] 路径中。历史报表表
     * （BodyMetricHistory / ParentReport / StudentDietRecord / ArchivedLesson）
     * 原样保留，通过 studentName 仍能查到该学员的历史记录。
     * 日常 UI 通过 [StudentDao.getAll] 自动过滤 isActive=0 的行。
     *
     * @param studentId 学员唯一 ID（必须非空）
        ...
        if (database != null) {
            ...
            try {
                database.withTransaction {
                    ...
                    // v47 级联清理 4 张业务子表（双通道：studentId 优先，旧数据 NULL 回退姓名）
                    database.lessonDao().deleteByStudentIdDual(studentId, target.name)
