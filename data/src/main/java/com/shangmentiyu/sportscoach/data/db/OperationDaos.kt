...
interface LessonPackageDao {
    ...
    /**
     * v46：双通道查询（studentId 优先，studentName 回退，兼容 v20 前旧数据 studentId=NULL）。
     * 同一行只返回一次，不会重复计数。
     */
    @Query("SELECT * FROM lesson_packages WHERE studentId = :studentId OR (studentId IS NULL AND studentName = :name) ORDER BY purchaseDate DESC")
    fun getByStudentDual(studentId: String?, name: String): Flow<List<LessonPackage>>
    ...
    /**
     * 删除学员的所有课时包（双通道：studentId 优先，旧数据 studentId=NULL 回退按姓名）。
     * 用于 [StudentRepository.softDeleteStudentById] 级联删除场景，避免同名误删 / 旧数据漏删。
     */
    @Query("DELETE FROM lesson_packages WHERE studentId = :studentId OR (studentId IS NULL AND studentName = :name)")
...
interface ScheduleDao {
     ...
     * 同一行只返回一次，不会重复计数。
     */
    @Query("SELECT * FROM schedules WHERE isActive = 1 AND (studentId = :studentId OR (studentId IS NULL AND studentName = :name)) ORDER BY dayOfWeek, startTime")
    fun getByStudentDual(studentId: String?, name: String): Flow<List<Schedule>>
    ...
    /**
     * 删除学员的所有排课（双通道：studentId 优先，旧数据 studentId=NULL 回退按姓名）。
     * 用于 [StudentRepository.softDeleteStudentById] 级联删除场景，避免同名误删 / 旧数据漏删。
     */
    @Query("DELETE FROM schedules WHERE studentId = :studentId OR (studentId IS NULL AND studentName = :name)")
    suspend fun deleteByStudentIdDual(studentId: String, name: String)
    ...
    /** 双通道：删除该学员 startDate 早于指定日期的全部排课（修正历史排课通道1）。体验课无购买日期约束，不参与清理 */
    @Query("DELETE FROM schedules WHERE (studentId = :studentId OR (studentId IS NULL AND studentName = :name)) AND startDate < :dateStr AND isTrial = 0")
