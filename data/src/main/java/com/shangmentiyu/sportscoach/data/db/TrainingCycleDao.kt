...
interface TrainingCycleDao {
    ...
    /**
     * 删除学员的所有训练周期（双通道：studentId 优先，旧数据 studentId=NULL 回退按姓名）。
     * 用于 [StudentRepository.softDeleteStudentById] 级联删除场景，避免同名误删 / 旧数据漏删。
     */
    @Query("DELETE FROM training_cycles WHERE studentId = :studentId OR (studentId IS NULL AND studentName = :name)")
